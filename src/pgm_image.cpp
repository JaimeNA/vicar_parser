...
PgmImage::PgmImage(const std::string filename, std::size_t width, std::size_t height, std::size_t max_value) {
    ...
    if (!file) {
        std::cerr << "ERROR::PGM_IMAGE: Error opening file!" << std::endl;
    }
...
void PgmImage::put_pixel(std::size_t x, std::size_t y, uint16_t value) {
...
void PgmImage::draw_square(std::size_t x, std::size_t y, uint64_t size, uint16_t value) {

    // Place it in the file
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < size; j++) {
            put_pixel(x+j, y+i, value);
		}
	}
}

/* Draws char at provided coordinates(top-left corner of char) */
void PgmImage::draw_char(char c, std::size_t font_size, std::size_t x, std::size_t y, uint16_t value) {
	...
	for (int i = 0; i < CHAR_BITMAP_HEIGHT; i++) {
		for (int j = 0; j < CHAR_BITMAP_WIDTH; j++) {
			...
			if (pixel >> (CHAR_BITMAP_WIDTH-j-1) & 0x01)  // If there should be a "pixel" from bitmap drawn
				draw_square(x + j*bitmapPixelSize, y + i*bitmapPixelSize, bitmapPixelSize, value);
		}
	}
...
void PgmImage::draw_text(const std::string str, std::size_t font_size, std::size_t x, std::size_t y, uint16_t value) {
